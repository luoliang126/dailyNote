...
var addToOperationsList = function(_self, docType, document) {
  ...
  // Throw error if the doc is bigger than the max BSON window-object
  if(bsonSize >= _self.s.maxBatchSizeBytes) {
		throw toError("document is larger than the maximum window-object " + _self.s.maxBatchSizeBytes);
	}
  ...
  if(((_self.s.currentBatchSize + 1) >= _self.s.maxWriteBatchSize)
    ...
    // Reset the current window-object trackers
    _self.s.currentBatchSize = 0;
    _self.s.currentBatchSizeBytes = 0;
  } else {
    // Update current batch window-object
...
function OrderedBulkOperation(topology, collection, options) {
  ...
  this.s = {
    ...
    // Max batch window-object options
