    ...
    <ol>
        <li>
            ...
            <pre>
                ...
                outer(); // 1 这是一个一般的函数，每执行一次outer函数时，都会在outer作用域下var localVal = 1，当执行完成后，因为localVal没有被再使用，所以就会被垃圾回收而清除；下一次执行时再创建var localVal = 1，依次类推
            </pre>
            <pre>
                function outer(){
                    var localVal = 1;
                ...
                var func = outer();
                func(); //1 这里形成了闭包，当定义var func = outer()时，执行outer函数，var localVal = 1并return一个function(){return localVal},但并未执行该function，重点来了，该function中会用到localVal，所以localVal不会被释放，而是常驻内存中，这样闭包形成了；
            </pre>
        </li>
            ...
            <ul>
                ...
                <li>
                    ...
                    <ol>
                        ...
                        <li>
                            第一次执行result()时，就相当于f1()();那么执行的是return出来的f2函数，所以alert(n) //999，但内存中的n并没有被消除
                        </li>
                        <li>
                            第一次nAdd()时，此时n+=1；使用的n是活动变量n（未被清除，常驻内存中的n），最终n变为1000，但依然没有被清除掉。
                        </li>
                        <li>
                            第二次执行result()时，就相当于f1()();执行f2函数，但使用的n是内存中的n=1000;如果再执行f1()函数时，依然会使用n=1000；
                        </li>
                        <li>
                            第二次nAdd()时，此时n+=1；使用的n依然是活动变量n（未被清除，常驻内存中的n），最终n变为1001，但依然没有被清除掉。
                        </li>
